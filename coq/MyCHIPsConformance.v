...
Module Conformance.
  ...
  Definition acts_valid (size : nat) (acts : list Action) :=
    size > 1 /\
  ...
  Example basic_size_3_trace_valid : acts_valid 3 [(Send 0 1 Promise) ; (Receive 1 Promise) ; (Send 1 2 Promise) ; (Receive 2 Promise) ; (Send 2 3 Promise) ; (Receive 0 Promise) ; (Send 0 (-1) Commit); (Receive 2 Commit); (Send 2 1 Commit) ; (Receive 1 Commit) ; (Send 1 0 Commit) ; (Receive 0 Commit)].
  Proof.
    unfold acts_valid.
    split.
    lia.
  ...
  Qed.
    ...
    Lemma valid_implies_all_receives_causal_in_proj : forall (acts : list Action) (n : nat), acts_valid (S n) acts -> n > 1 -> all_receives_causal (project_to_size n acts) n.
    Proof.
  ...
  Lemma larger_conforms_to_smaller : forall (acts : list Action) (n : nat), acts_valid (S n) acts  /\ n > 1 -> acts_valid n (project_to_size n acts).
    Proof.
      intros.
      destruct H.
      assert (Hd := H).
  ...
  Lemma projected_larger_conforms_to_smaller : forall (acts : list Action) (n : nat), acts_valid (S n) (project_to_size (S n) acts) /\ n > 1 -> acts_valid n (project_to_size n acts).
    Proof.
      intros.
      destruct H.
      assert (Hd := H).
    ...
    Theorem all_valid_systems_conform_to_size_3 : forall (acts : list Action) (n : nat), acts_valid n acts /\ n > 3 -> acts_valid 3 (project_to_size 3 acts).
    Proof.
      intros.
      destruct H.
      destruct n.
      ...
      assumption.
      lia.
      assert (acts_valid (S (S (S (S n)))) (project_to_size (S (S (S (S n)))) acts) ).
      apply larger_conforms_to_smaller.
      split.
      assumption.
      lia.
    ...
    Definition System := (list Action -> Prop). (*Each system is defined by the set of valid actions in the system*)
    Definition Projection := list Action -> list Action. (*A projection transforms one list of actions to another*)
    Definition Conforms (a : System) (b : System) (proj_to_b : Projection) :=
      forall (acts : list Action), (a acts) -> (b (proj_to_b acts)).
    (*Given the system a and the system b and the projection that converts actions in the alphabet of a into the alphabet of b, a conforms to b if the set of actions accepted once projected are equivilant*)

    (* Result of Dill's work restated in this context*)
    Axiom conformance_implies_property_pairity : forall (SysProp : System -> Prop) (a : System) (b : System) (proj_to_a : Projection), SysProp a /\ (Conforms b a proj_to_a) -> SysProp b.

    Definition system_requirements : System -> Prop.
    Proof.
    intros.
    apply False. (*A placeholder here for system requirements that are difficult to prove*)
    Qed.


    (* Result of this work's SPIN model *)
    Axiom system_of_size_3_meets_requirements: (system_requirements (acts_valid 3)).

    Theorem systems_of_any_size_meet_requirements: forall n, n > 3 -> system_requirements (acts_valid n).
    Proof.
      intros.
      apply conformance_implies_property_pairity with (acts_valid 3) (project_to_size 3).
      split.
      apply system_of_size_3_meets_requirements.
      unfold Conforms.
      intros.
      apply all_valid_systems_conform_to_size_3 with n.
      auto.
    Qed.
