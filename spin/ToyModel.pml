#define N_NODES 3 
chan succ[N_NODES] = [10] of { byte }
chan pred[N_NODES] = [10] of { byte }
byte state[N_NODES]
...
inline sendToPred(value, id) {
    ...
    :: true ->
        ...
        succ[prev]!value; //put it in their succ box (if they are my pred I am their succ)
    :: true ->
...
proctype originator() {
  state[ORIGINATOR] = START
  do
  :: (state[ORIGINATOR] == START) ->
    printf("Originator: Initiating Lift\n")
    true -> sendToPred(sum, ORIGINATOR)
